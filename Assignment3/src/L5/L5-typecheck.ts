...
import { mapv ,Result, makeFailure, bind, makeOk, zipWithResult, either } from '../shared/result';
...
const isTypePredApp = (e: Exp, tenv: TEnv): Result<TEnv[]> => {
    ...
    if (!isAppExp(e))
     {
    return makeFailure('type predicate must be an application')
    }

    const rand = e.rands[0];//rand t
    // const rator = e.rator;
    // const rands = e.rands;
    if (!isVarRef(rand))
        
    return makeFailure(`Not a variable reference: ${rand}`);
    return bind(typeofApp(e, tenv),(te: TExp) => 
                bind(typeofExp(rand, tenv), (ratorTE: TExp) =>
                    isTypePredTExp(te) ? (makeOk([makeExtendTEnv([rand.var], [te.type], tenv),
                                        ...
                                        makeFailure(`Not a type predicate: ${te}`)));//rand type


                                        // return makeFailure(`Not a type predicate: ${te}`));
} 
...
export const typeofIf = (ifExp: IfExp, tenv: TEnv): Result<TExp> =>
    either(
        bind(isTypePredApp(ifExp.test, tenv), (newEnvs: TEnv[]) => 
            {
            //get type of test : (if----then --)
            const then = typeofExp(ifExp.then, newEnvs[0]);

            const alt = typeofExp(ifExp.alt, newEnvs[1]);
            // const constraint1 = bind(testTE, testTE => checkCompatibleType(testTE, makeBoolTExp(), ifExp));
            //bind(thenTE, (thenTE: TExp) => bind(altTE, (altTE: TExp) => makeOk(makeUnion(thenTE, altTE))));
            return bind(then, (thenTE: TExp) =>
                bind(alt, (altTE: TExp) =>
                    makeOk(makeUnion(thenTE, altTE))));
        }),
        makeOk,
