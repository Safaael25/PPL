...
//L5 added anyTExp and neverTExp:
...
export const isCompoundTExp = (x: any): x is CompoundTExp => isProcTExp(x) || isTupleTExp(x) || isUnionTExp(x) ||isTypePredTExp(x)|| isInterTExp(x) ; 
...
export const isSubType = (te1: TExp, te2: TExp): boolean =>
    ...
    isInterTExp(te2) ? containType(te2.components, te1) :
   ...
   // isInterTExp(te2) ? te2.components.some((c) => isSubType(te1, c)) : 
    ...
    false;
// ========================================================
export const containType = (tes: TExp[], te: TExp): boolean =>
    isEmpty(tes) ? false:
...
//added:
    const wrapInterTExp = (tes: string[]): string =>
        (tes.length == 1) ? tes[0] : 
        `(inter ${tes[0]} ${wrapInterTExp(tes.slice(1))})`
