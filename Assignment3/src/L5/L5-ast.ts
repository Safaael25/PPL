...
//}
    ...
    if (isArray(vars)) 
        ...
        const bodyIndex = rest[0] != ":" ? 0 : rest[1] != "is?" ? 2 : 3 ;
        const computedTExp = bodyIndex == 0 ? makeOk(makeFreshTVar()) :
                        bodyIndex == 2 ? parseTExp(rest[1]) : parseTExp(rest.slice(1, bodyIndex));
        //
        const body = mapResult(parseL5CExp, rest.slice(bodyIndex)); 
        return bind(args, (args: VarDecl[]) =>
                    bind(body, (body: CExp[]) =>
                        mapv(computedTExp, (returnTE: TExp) =>
