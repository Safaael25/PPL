...
const applyClosure = (proc: Closure, args: Value[], env: Env): Result<Value> => {
    ...
    const arg : CExp[] = map(valueToLitExp, args);
    return evalSequence(substitute(body, vars, arg), env);
}
//L31 :
...
const applyClass = (classValue: ClassValue, args: Value[], env: Env): Result<Value> =>
    ...
    const arg : CExp[] = map(valueToLitExp, args);
    const fields = map((f: VarDecl) => f.var, classValue.fields);//get the fields 
...
//    const methods = mapResult((b: Binding) => L3applicativeEval(b.val, env), classValue.methods);
    const closures = bind ( methods,
         ( methods: List<SExpValue>) => mapResult(
            (method: SExpValue): Result<Closure> => isClosure(method) ? makeOk(makeClosure(method.params, substitute(renameExps(method.body), fields, arg))) : makeFailure(`couldn't call ${method}`),
             methods));
//return makeObject(zipWith(makeBinding, methodsNames, closures));
...
const applyObject = (object: Object, args: Value[], env: Env): Result<Value> => 
    ...
    if(!isNonEmptyList<Value>(args))
        {
        return makeFailure(" makeFailur: No method name was given");
    }
    //check if the first argument is a symbol
    if (!isSymbolSExp(args[0]))
         {
        return makeFailure(" makeFailur:first argument isn't method name")
    }
...
//    return L3applicativeEval(selectedMethod.val, env);
