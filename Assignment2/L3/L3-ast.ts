...
export const parseL3SpecialForm = (op: Sexp, params: Sexp[]): Result<CExp> =>
    ...
    op === "quote" ? 
        ...
        //L31
    op === "class" ?
        isNonEmptyList<Sexp>(params) ? parseClassExp(first(params), second(params)): //check if not empty
        makeFailure(`Bad class exp: ${params}`) : //if empty
    makeFailure("Never");
...
//    const parsedFields = map(b => makeVarDecl(b), fields);
    ...
    const bindingsResult = mapv(procsResult, (procs: CExp[]) => zipWith(makeBinding, vars, procs));
//    return makeClassExp(parsedFields, bindingsResult);
    return mapv(bindingsResult, (bindings: Binding[]) => makeClassExp(parsedFields, bindings));
